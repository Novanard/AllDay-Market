<?php 
   if ($_SERVER['REQUEST_METHOD'] === "POST" ) {   
     ...
     session_start();
     if(isset($_POST['startMaking'])){
          $eID = $_POST['eID'];
          $barcode = $_POST['barcode'];
          $orderID = $_POST['orderID'];
          $sql = "UPDATE order_details set eID = ? WHERE itemBarcode = ? AND order_id = ?; ";
          $stmt= mysqli_stmt_init($conn);
          mysqli_stmt_prepare($stmt,$sql);
          mysqli_stmt_bind_param($stmt,"iii",$eID,$barcode,$orderID);
          mysqli_stmt_execute($stmt);
          header('Location:employeeOrders.php');
     }
    else if(isset($_POST['isDone'])){
         $barcode = $_POST['barcode'];
         $orderID = $_POST['orderID'];
         ...
         if(mysqli_num_rows($results)==0){
           ...
           while($row = mysqli_fetch_assoc($results)){
             ...
             $isDone = $row['isDone'];
             $img = $row['img'];
             $sql = "INSERT INTO oldOrder_details (id,itemBarcode,itemName,depNum,price,quantity,total,order_id,eID,isDone,img) VALUES (?,?,?,?,?,?,?,?,?,?,?);";
             $stmt= mysqli_stmt_init($conn);
             mysqli_stmt_prepare($stmt,$sql);
             mysqli_stmt_bind_param($stmt,"iisiiiiiiis",$id,$itemBarcode,$itemName,$depNum,$price,$quantity,$total,$orderID,$eID,$isDone,$img);
             mysqli_stmt_execute($stmt); 
